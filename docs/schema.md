...
### tracker_taskblocker
- `id` — auto integer primary key
- `blocked_task_id` — FK to tracker_task (CASCADE — deleting a task removes all its blocker rows)
- `blocking_task_id` — FK to tracker_task (CASCADE — same)
...
- `id` — auto integer primary key
- `task_id` — FK to tracker_task (CASCADE — deleting a task removes its dismissal records)
- `user_id` — FK to tracker_user (CASCADE — deleting a user removes their dismissals, unlike HistoryEntry/Comment which use SET_NULL)
...
- Project access (manager, or a `ProjectMembership` row) — depends on who is asking, so it cannot be a column constraint; enforced once in `ProjectAccessMixin` rather than per view
- Only an assigned user may dismiss a task's alert — requires reading `TaskAssignment`, so it lives in the view
- Only project members may be assigned to a task — checked in `assign_user_to_task`; the DB would happily insert the row
- A task cannot block itself and cannot be blocked by a task in another project — the form restricts the queryset; the `TaskBlocker` table itself would accept either
- **Not enforced anywhere:** blocker *cycles*. A can block B while B blocks A. Both rows are individually valid and both tasks simply become unable to reach Done. Catching this needs a graph traversal at write time (see "what breaks first at 100x")
