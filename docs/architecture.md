...
Locally: everything runs on your machine. Django's dev server (`uv run python manage.py runserver`) listens on port 8000. SQLite is a file on disk.

In production: the Django app runs on Railway as a single web process. The database is Railway's managed Postgres. Static files are served by WhiteNoise (a Python library that lets Django serve its own static files without a separate CDN or web server).
